...
function buildEmbed(event) {
  ...
  // Resolves, Category, Lines stacked cleanly — one per row (inline: false)
  embed.addFields(
    { name: "⏱️ Resolves", value: endDate, inline: false }
  );
  if (category) {
    embed.addFields({ name: "🏷️ Category", value: category, inline: false });
  }
  embed.addFields({ name: "📋 Lines", value: marketLabel, inline: false });
