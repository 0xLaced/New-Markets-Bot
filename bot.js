...
async function pollMarkets() {
  ...
  const newEvents = events.filter((e) => {
    ...
    // Skip short-duration crypto up/down contracts (BTC/ETH 5-min, 1-min lines).
    // These resolve in under 30 minutes and would spam constantly.
    const endRaw = e.endDate ?? e.markets?.[0]?.endDate ?? null;
    const startRaw = e.startDate ?? e.createdAt ?? null;
    if (endRaw && startRaw) {
      const durationMs = new Date(endRaw) - new Date(startRaw);
    ...
    // Also catch them by title keywords as a safety net
    const title = (e.title || e.question || "").toLowerCase();
    const skipKeywords = ["5-minute", "5 minute", "5min", "1-minute", "1 minute", "1min"];
    if (skipKeywords.some((kw) => title.includes(kw))) {
      console.log("[NewMarkets] Skipping keyword-matched event: " + (e.title || e.id));
      seenEventIds.add(e.id);
      return false;
    }
